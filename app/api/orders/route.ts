...
export async function GET(request: Request) {
  ...
  const orderId = url.searchParams.get("id")?.trim().toUpperCase();
  if (!orderId || !/^VPO\d{6}$/.test(orderId)) {
...
export async function POST(request: Request) {
  try {
    ...
    if (String(payload.paymentMethod) !== "Cash on Delivery") return Response.json({ error:"Online payments must be verified through Razorpay." },{status:400});
    const orderId = `VPO${Math.floor(100000 + Math.random() * 900000)}`;
    const db = await getDb();
    const ownerEmail = request.headers.get("oai-authenticated-user-email")?.toLowerCase() || null;
