...
export async function POST(request:Request){
 ...
 const message=appreciation(name,amount);const db=await getDb();await db.insert(donations).values({donationId,donorName:name.slice(0,100),email:email.slice(0,150),mobile:mobile.slice(0,20),amount,paymentMethod:`Razorpay • ${paymentId}`.slice(0,30),paymentStatus:"Verified",certificateId,appreciationMessage:message});
 await saveDonationToSupabase({donation_id:donationId,donor_name:name.slice(0,100),email:email.slice(0,150),mobile:mobile.slice(0,20),amount,payment_method:`Razorpay • ${paymentId}`.slice(0,30),payment_status:"Verified",certificate_id:certificateId,appreciation_message:message});
 return Response.json({donationId,certificateId,donorName:name,amount,appreciationMessage:message,paymentStatus:"Verified"},{status:201})
